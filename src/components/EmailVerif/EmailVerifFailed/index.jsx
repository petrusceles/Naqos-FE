...
import EmailVerifFailedModal from "./modal";
function EmailVerifFailed() {
  const [isEmailVerifFailedModalShow, setIsEmailVerifFailedModalShow] =
    useState(false);
  return (
    <>
      {isEmailVerifFailedModalShow && (
        <EmailVerifFailedModal
          setIsModalShow={setIsEmailVerifFailedModalShow}
        />
      )}
      <div className="container grid grid-cols-1 gap-24">
        ...
        <div className="w-full flex flex-wrap gap-2 lg:gap-5 justify-center pb-28">
          ...
          <button className="mt-5 w-[90%] lg:w-1/2 rounded-full bg-[#0A008A] capitalize hover:bg-[#E1E0FF] hover:text-[#4D50C5] text-white py-3 lg:text-xl lg:font-semibold" onClick={() => {setIsEmailVerifFailedModalShow(true)}}>
